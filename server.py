...
async def handle_client(reader, writer):
    ...
    elif method=='GET' and route=='/arena/sprites.js':
        sprites_path=os.path.join(os.path.dirname(__file__),'arena_sprites.js')
        try:
            with open(sprites_path,'r') as sf: sprites_data=sf.read()
            await send_http(writer,200,sprites_data,'application/javascript; charset=utf-8')
        except FileNotFoundError:
            await send_http(writer,404,'// sprites not found','application/javascript')
...
<script src="/arena/sprites.js"></script>
<script>
...
// ═══ PROFESSIONAL SPRITE RENDERING ENGINE ═══
// Uses CraftPix sprite sheets loaded from arena_sprites.js
// 6 characters: woodcutter, graverobber, steamman, bloodfang, shadow, ironclaw

const CHAR_MAP = {
  '블러드팡': 'bloodfang', '아이언클로': 'ironclaw', '쉐도우': 'shadow',
  '버서커': 'steamman', '나무꾼': 'woodcutter', '도굴꾼': 'graverobber', '스팀맨': 'steamman'
};

// Animation state → spritesheet key mapping
const ANIM_MAP = {
  'idle': 'idle', 'run': 'run', 'attack': 'attack1', 'heavy_attack': 'attack2',
  'special': 'attack3', 'block': 'idle', 'hit': 'hurt', 'dodge': 'run',
  'dead': 'death', 'jump': 'jump', 'aerial_attack': 'attack1', 'walk': 'walk'
};

// Pre-load all spritesheet images
const _sheetImgs = {};
const _sheetLoaded = {};
(function loadSheets() {
  if (typeof ARENA_SPRITES === 'undefined') { console.warn('ARENA_SPRITES not loaded yet'); return; }
  for (const [name, anims] of Object.entries(ARENA_SPRITES)) {
    _sheetImgs[name] = {};
    for (const [anim, url] of Object.entries(anims)) {
      const img = new Image();
      img.onload = function() { _sheetLoaded[name + '_' + anim] = true; };
      img.onerror = function() { console.error('Sheet FAILED:', name, anim); };
      img.src = url;
      _sheetImgs[name][anim] = img;
    }
  }
})();

function _getSheet(fighterName) {
  const mapped = CHAR_MAP[fighterName];
  if (mapped && _sheetImgs[mapped]) return { sheets: _sheetImgs[mapped], name: mapped };
  // Try direct name match
  if (_sheetImgs[fighterName]) return { sheets: _sheetImgs[fighterName], name: fighterName };
  // Fallback: assign based on hash of name
  const names = Object.keys(_sheetImgs);
  if (names.length === 0) return null;
  let h = 0; for (let i = 0; i < fighterName.length; i++) h = (h * 31 + fighterName.charCodeAt(i)) & 0xFFFF;
  return { sheets: _sheetImgs[names[h % names.length]], name: names[h % names.length] };
}
...
const PX = 3; // kept for gore parts
function px(x, y, w, h) { ctx.fillRect(x * PX, y * PX, (w || 1) * PX, (h || 1) * PX) }

const FW = (typeof FRAME_W !== 'undefined') ? FRAME_W : 48;
const FH = (typeof FRAME_H !== 'undefined') ? FRAME_H : 48;
const FC = (typeof FRAME_COUNTS !== 'undefined') ? FRAME_COUNTS : {idle:4, run:6, attack1:6, attack2:6, attack3:6, jump:6, hurt:3, death:6, walk:6};
...
function drawFighter(f, tick) {
  try {
    ...
    const col = f.color;
    const sheetInfo = _getSheet(f.name);
    ...
    // Map animation state to spritesheet key
    const animKey = ANIM_MAP[anim] || ANIM_MAP['idle'];
    const frameCount = FC[animKey] || 4;

    // Calculate current frame
    const animSpeed = (anim === 'attack' || anim === 'heavy_attack' || anim === 'special') ? 6 : 
                      (anim === 'hit') ? 8 : 5;
    const frameIdx = Math.floor(tick / animSpeed) % frameCount;

    // Get sprite sheet image
    const sheet = sheetInfo ? sheetInfo.sheets[animKey] : null;
    const SCALE = 2.8; // 48px * 2.8 = ~134px on screen
    ...
    ctx.scale(face, 1);
    // drawImage with spritesheet slicing: source rect = (frameIdx*FW, 0, FW, FH)
    ctx.drawImage(sheet, frameIdx * FW, 0, FW, FH, -sw / 2, -sh, sw, sh);
    ctx.restore();

    // Color tint for non-mapped fighters (external bots)
    const hasOwnSprite = !!CHAR_MAP[f.name] || (sheetInfo && _sheetImgs[f.name]);
    if (!hasOwnSprite) {
    ...
    if (anim === 'dodge') {
      ...
      ctx.imageSmoothingEnabled = false;
      const idleSheet = sheetInfo ? sheetInfo.sheets['idle'] : null;
      if (idleSheet && idleSheet.complete) {
        ctx.drawImage(idleSheet, 0, 0, FW, FH, -sw / 2, -sh, sw, sh);
      }
      ctx.restore(); ctx.globalAlpha = 1;
    }

    // Attack slash effect
    if ((anim === 'attack' || anim === 'heavy_attack' || anim === 'special' || anim === 'aerial_attack') && f.attack_ticks > 1) {
      ctx.globalAlpha = 0.35;
      ctx.strokeStyle = anim === 'special' ? '#ffff00' : '#ffffff';
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(gx + face * 50, gy - sh / 2 + bobY, 30, -1.2, 1.2);
      ctx.stroke();
      ctx.globalAlpha = 0.5; ctx.fillStyle = '#ffff44';
      ctx.beginPath(); ctx.arc(gx + face * 60, gy - sh / 2 + bobY, 10 + Math.random() * 8, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1;
    }
    ...
    if (anim === 'block') {
      ...
      ctx.beginPath(); ctx.arc(gx + face * 14, gy - sh / 2 + bobY, 34, 0, Math.PI * 2); ctx.stroke();
      ctx.strokeStyle = 'rgba(100,180,255,0.2)'; ctx.lineWidth = 7;
      ctx.beginPath(); ctx.arc(gx + face * 14, gy - sh / 2 + bobY, 40, 0, Math.PI * 2); ctx.stroke();
    }

    // Bleed indicator
    if (f.bleed_ticks > 0) {
      ctx.fillStyle = '#ff0000'; ctx.font = '14px Jua'; ctx.textAlign = 'center';
      ctx.fillText('🩸', gx + face * 22, gy - sh + bobY - 10);
    }
    ...
    if (f.combo > 1) {
      ctx.font = 'bold 20px Jua'; ctx.fillStyle = '#ffaa00';
      ctx.fillText('🔥x' + f.combo, gx, headWorldY - 52);
    }
    if (f.special_gauge >= 100) {
      ctx.font = 'bold 15px Jua'; ctx.fillStyle = '#ffff00';
      ctx.fillText('⚡ SPECIAL', gx, headWorldY - 68);
    }
    if (f.stun_ticks > 0) {
      ctx.fillStyle = '#ffff44'; const starA = tick * 0.3;
      for (let i = 0; i < 3; i++) {
    ...
    console.error('drawFighter ERROR:', e.message, e.stack);
    ctx.fillStyle = f.color || '#fff'; ctx.fillRect(f.x - 15, (GROUND + f.y) - 80, 30, 80);
    ctx.font = '11px Jua'; ctx.textAlign = 'center'; ctx.fillStyle = '#fff';
    ctx.fillText(f.name || '?', f.x, (GROUND + f.y) - 85);
  }
}
...
function drawGoreParts(f, tick) {
  ...
  for (const gp of f.gore_parts) {
    ...
    // Try to use sprite-based gore: grab portion of death sprite
    const sheetInfo = _getSheet(f.name);
    const deathSheet = sheetInfo ? sheetInfo.sheets['death'] : null;
    const hasDeathSprite = deathSheet && deathSheet.complete && deathSheet.naturalWidth;

    if (gp.type === 'head' && hasDeathSprite) {
      // Draw head portion from last death frame
      const lastFrame = (FC.death || 6) - 1;
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(deathSheet, lastFrame * FW, 0, FW, FH / 2, -30, -30, 60, 30);
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-20, 20, 40, 8); // blood drip
    } else if (gp.type === 'head') {
      ctx.fillStyle = c;
      for (let py = -5; py < 5; py++) for (let px = -4; px < 4; px++) {
        if (py < -2) ctx.fillStyle = dk;
        else if (py < 2) ctx.fillStyle = '#e8c090';
        else ctx.fillStyle = c;
        ctx.fillRect(px * P, py * P, P, P);
      }
      ctx.fillStyle = '#fff'; ctx.fillRect(P, -P, P, P);
      ctx.fillStyle = '#111'; ctx.fillRect(P + 1, -P + 1, P - 1, P - 1);
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-3 * P, 4 * P, 6 * P, 2 * P);
    } else if (gp.type === 'arm') {
      ctx.fillStyle = c;
      for (let py = -4; py < 4; py++) ctx.fillRect(-P, py * P, 3 * P, P);
      ctx.fillStyle = '#e8c090'; ctx.fillRect(-P, 3 * P, 3 * P, 2 * P);
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-P, -4 * P, 3 * P, 2 * P);
    } else if (gp.type === 'upper' && hasDeathSprite) {
      const lastFrame = (FC.death || 6) - 1;
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(deathSheet, lastFrame * FW, 0, FW, FH / 2, -40, -40, 80, 40);
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-40, 30, 80, 8);
    } else if (gp.type === 'upper') {
      ctx.fillStyle = c;
      for (let py = -8; py < 2; py++) for (let px = -4; px < 4; px++) {
        if (py < -4) ctx.fillStyle = '#e8c090';
        else ctx.fillStyle = c;
        ctx.fillRect(px * P, py * P, P, P);
      }
      ctx.fillStyle = dk; ctx.fillRect(-4 * P, -10 * P, 8 * P, 2 * P);
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-4 * P, P, 8 * P, 2 * P);
    } else if (gp.type === 'lower') {
      ctx.fillStyle = dk;
      for (let py = 0; py < 6; py++) for (let px = -3; px < 3; px++) {
        if (py > 3) ctx.fillStyle = '#443322';
        else ctx.fillStyle = dk;
        ctx.fillRect(px * P, py * P, P, P);
      }
      ctx.fillStyle = '#cc0000'; ctx.fillRect(-3 * P, -P, 6 * P, 2 * P);
